    ...
    items: [
      {
        title: "Fast Fire",
        icon: "mdi:fire",
        content: "## Overview\nThe \"Fast Fire\" feature enables rapid, automated firing for specific weapons in DDRaceNetwork, bypassing the default firing rate limitations. This is achieved by toggling the fire input at a high frequency when the fire key is held, creating a continuous firing effect for eligible weapons.\n\n## Implementation\nThe following code snippet implements the Fast Fire feature. It should be integrated into the `CControls::SnapInput` method in `src/game/client/components/controls.cpp`, just before the line `m_aLastData[g_Config.m_ClDummy] = m_aInputData[g_Config.m_ClDummy];`.\n\n```cpp\nif(g_Config.m_ClFastFire &&\n    (GameClient()->m_Snap.m_pLocalCharacter &&\n    GameClient()->m_Snap.m_pLocalCharacter->m_Weapon != WEAPON_SHOTGUN &&\n    GameClient()->m_Snap.m_pLocalCharacter->m_Weapon != WEAPON_GRENADE &&\n    GameClient()->m_Snap.m_pLocalCharacter->m_Weapon != WEAPON_LASER &&\n    GameClient()->m_Snap.m_pLocalCharacter->m_Weapon != WEAPON_NINJA) &&\n    !(GameClient()->m_aClients[GameClient()->m_Snap.m_LocalClientId].m_Jetpack && GameClient()->m_Snap.m_pLocalCharacter->m_Weapon == WEAPON_GUN) &&\n    m_aInputData[g_Config.m_ClDummy].m_PlayerFlags != PLAYERFLAG_IN_MENU)\n{\n    // Get the key bound to +fire\n    char aFireKey[64] = {0};\n    GameClient()->m_Binds.GetKey(\"+fire\", aFireKey, sizeof(aFireKey));\n    int FireKeyId = Input()->FindKeyByName(aFireKey);\n\n    if(FireKeyId != KEY_UNKNOWN && Input()->KeyIsPressed(FireKeyId))\n    {\n        m_aInputData[g_Config.m_ClDummy].m_Fire ^= 1;\n    }\n}\n```",
      },
      {
        title: "Hook Spam",
        icon: "mdi:hook",
        content: "## Overview\nThe \"Hook Spam\" feature enables rapid, automated toggling of the hook action in DDRaceNetwork, allowing the player to repeatedly activate and deactivate the hook at a controlled interval (50 ms) when the hook key is held. The feature ensures the hook toggles frequently but avoids overloading the client or server by enforcing a minimum delay between toggles.\n\n## Implementation\nThe following code snippet implements the Hook Spam feature. It should be integrated into the `CControls::SnapInput` method in `src/game/client/components/controls.cpp`, just before the line `m_aLastData[g_Config.m_ClDummy] = m_aInputData[g_Config.m_ClDummy];`.\n\n```cpp\nif(g_Config.m_ClHookSpam)\n{\n    // Get the key bound to +hook\n    char aHookKey[64] = {0};\n    GameClient()->m_Binds.GetKey(\"+hook\", aHookKey, sizeof(aHookKey));\n    int HookKeyId = Input()->FindKeyByName(aHookKey);\n\n    // Check if the hook key is pressed\n    if(HookKeyId != KEY_UNKNOWN && Input()->KeyIsPressed(HookKeyId))\n    {\n        static int64_t LastHookTime = 0;\n        int64_t CurrentTime = time_get();\n        int64_t TimeFreq = time_freq();\n\n        // Add a 50 ms delay\n        if(CurrentTime > LastHookTime + TimeFreq / 20) // 50 ms = 1/20th of a second\n        {\n            m_aInputData[g_Config.m_ClDummy].m_Hook ^= 1; // Toggle hook state\n            LastHookTime = CurrentTime;\n        }\n    }\n}\n```"
      },
      {
        title: "Version Spoofer",
        icon: "lucide:git-commit",
        content: "## Overview\nThe \"Version Spoofer\" feature allows to send a custom client version to the server, bypassing version checks that might restrict access or detect modified clients. By altering the version information sent during the connection process, this feature can help the client appear as a different version of DDNet, potentially evading server-side anti-cheat mechanisms or compatibility restrictions. This is particularly useful for testing or accessing servers with specific version requirements.\n\n## Implementation\nThe following code snippet modifies the `CClient::SendInfo` method in `src/engine/client/client.cpp` to implement version spoofing. It replaces calls to `GameClient()->NetVersion()`, `GameClient()->DDNetVersion()`, and related methods with configurable spoofed values when the feature is enabled.\n\n```cpp\nvoid CClient::SendInfo(int Conn)\n{\n    CMsgPacker MsgVer(NETMSG_CLIENTVER, true);\n    MsgVer.AddRaw(&m_ConnectionId, sizeof(m_ConnectionId));\n\n    // Check if version spoofing is enabled\n    if(g_Config.m_ClSpoofVersion)\n    {\n        MsgVer.AddInt(g_Config.m_ClSpoofedVersionNumber); // Configurable spoofed version number\n        MsgVer.AddString(g_Config.m_ClSpoofedVersionString); // Configurable spoofed version string\n    }\n    else\n    {\n        MsgVer.AddInt(GameClient()->DDNetVersion());\n        MsgVer.AddString(GameClient()->DDNetVersionStr());\n    }\n\n    SendMsg(Conn, &MsgVer, MSGFLAG_VITAL);\n\n    if(IsSixup())\n    {\n        CMsgPacker Msg(NETMSG_INFO, true);\n        if(g_Config.m_ClSpoofVersion)\n        {\n            Msg.AddString(g_Config.m_ClSpoofedNetVersion7, 128); // Configurable spoofed network version for Sixup\n            Msg.AddString(Config()->m_Password); // Spoofed password\n            Msg.AddInt(g_Config.m_ClSpoofedClientVersion); // Configurable spoofed client version\n        }\n        else\n        {\n            Msg.AddString(GAME_NETVERSION7, 128);\n            Msg.AddString(Config()->m_Password);\n            Msg.AddInt(GameClient()->ClientVersion7());\n        }\n        SendMsg(Conn, &Msg, MSGFLAG_VITAL | MSGFLAG_FLUSH);\n        return;\n    }\n\n    CMsgPacker Msg(NETMSG_INFO, true);\n    if(g_Config.m_ClSpoofVersion)\n    {\n        Msg.AddString(g_Config.m_ClSpoofedNetVersion); // Configurable spoofed network version\n        Msg.AddString(m_aPassword); // Spoofed password\n    }\n    else\n    {\n        Msg.AddString(GameClient()->NetVersion());\n        Msg.AddString(m_aPassword);\n    }\n    SendMsg(Conn, &Msg, MSGFLAG_VITAL | MSGFLAG_FLUSH);\n}\n```",
      },
      {
